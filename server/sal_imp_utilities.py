...
#         print('generator: outs[0][0][0].shape (should be first map of batch)',outs[0][0][0].shape)
    ...
    #         labels = get_labels(batch_x, self.category_dict, self.str2label)
    #     else:
    ...
    #     print(f"Shapes - Images: {np.shape(images)}, Maps: {np.shape(maps)}, Fixs: {np.shape(fixs) if self.fix_filenames is not None else 'None'}, Labels: {np.shape(labels)}")
